...
- **Attribution / handle-map corrections (4/6 weeks).** Consult the existing team-roster / TEAM_CHARTER handle map when writing @mentions. Recurring examples: @lae→@mlaetitia, @nadiahussain→@thirtythreestudio, @jacklyn.lock→@jackiejade. **Shipped in v1.3.0** — weekly-update generator now loads `JOB_CONTEXT.md` (which carries the auto-synced Matticspace roster) and the system prompt tells it to consult the roster for @mentions or write plain names when a person isn't in the roster. Handle-correction **learn-loop** (auto-detecting "Smithers wrote @X, Katie changed to @Y, N weeks in a row" from the diff archive) deferred to Sprint 4 — non-blocking since the roster consultation covers most cases.
