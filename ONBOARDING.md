...
- **team51 CLI Provisioning workflows launch a Terminal window.** Smithers composes the command from the form you fill in, then AppleScripts Terminal.app to open a new window running it. The first time this happens macOS asks: **"Smithers wants to control Terminal.app — Allow / Deny?"** Click **Allow** (one-time — the OS remembers). The CLI runs interactively in that terminal so its prompts (Symfony's confirmation, 1Password's biometric) work naturally; when it finishes the terminal script POSTs the log back to Smithers automatically. **Leave the Terminal window open** until you see "Reported. Safe to close." If you close it mid-run the CLI still finishes (site actually gets created) but Smithers won't capture the URL for frontmatter write-back — the log is still on disk under `~/.smithers/team51-logs/<run-id>.log`.
