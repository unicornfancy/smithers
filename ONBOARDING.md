...
- **The in-app Restart Dev Server button breaks 1Password CLI auth.** Smithers's Restart button spawns the new `pnpm dev` with `detached: true` — the new process becomes a child of `launchd`, orphaned from the terminal. 1Password 8 walks the process ancestry to decide which caller can trigger biometric prompts; without a Terminal ancestor, `op` refuses the call **silently** (no prompt appears) and Provisioning workflows all fail with `external-auth-failed:op`. **For provisioning-heavy days, launch `pnpm dev` directly from a Terminal window and restart it with `Ctrl+C` + `pnpm dev` in that same terminal — don't use the in-app Restart button.** For config-refresh restarts that don't need `op`, the button is fine.
